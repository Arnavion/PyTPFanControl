	...
	def __init__(self):
		...
		if not Fan.isWritable():
			...
			try:
				...
				notifications.Notify('PyTPFanControl', dbus.UInt32(0), '', 'Warning', 'TPFanControl does not have write access to the ACPI interface. Fan speed will be read-only.', dbus.Array(signature='s'), dbus.Dictionary(signature='sv'), 0)
